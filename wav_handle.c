...
int wav_parse_header(wav_handle_t *h)
{
    ...
    if (header.audio_format != 1) { // PCM
        ESP_LOGE(TAG, "bad audio_format: %u", header.audio_format);
        return -1;
    }

    if (header.sample_rate < 8000 || header.sample_rate > 44100) {
        ESP_LOGE(TAG, "bad sample_rate = %u", header.sample_rate);
        return -1;
    }
    ...
    ESP_LOGD(TAG, "num_channels=%u", header.num_channels);
    ESP_LOGD(TAG, "sample_rate=%u", header.sample_rate);
    ESP_LOGD(TAG, "byte_rate=%u", header.byte_rate);
    ESP_LOGD(TAG, "sample_alignment=%u", header.sample_alignment);
    ESP_LOGD(TAG, "bit_depth=%u", header.bit_depth);
    ESP_LOGD(TAG, "data_bytes=%u", header.data_bytes);
